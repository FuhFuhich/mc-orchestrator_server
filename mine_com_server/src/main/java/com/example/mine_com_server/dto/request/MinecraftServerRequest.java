...
public class MinecraftServerRequest {
    ...
    @Pattern(
            regexp = "^\\d+\\.\\d+(\\.\\d+)?$",
            message = "Версия Minecraft должна быть в формате 1.20 или 1.20.1"
    )
    private String minecraftVersion;

    @Pattern(
            regexp = "^(vanilla|forge|fabric|paper|spigot|bukkit)?$",
            message = "Мод-лоадер: vanilla, forge, fabric, paper, spigot, bukkit"
    )
    ...
    @Pattern(
            regexp = "^(screen|docker)?$",
            message = "Цель деплоя: screen или docker"
    )
